...
for i in range(num_dividas):
    ...
    DIVIDAS.append({"valor": valor, "juros": juros})

PAGAMENTO_MENSAL = st.number_input("Pagamento Mensal", min_value=0.0, value=10_000.0, step=1_000.0)
...
for m in range(1, int(TEMPO) + 1):
    ...
    payed = False
    next_pay = PAGAMENTO_MENSAL
    for divida in DIVIDAS:
        if divida["valor"] > 0:
            if not payed:
                divida["valor"] -= next_pay
                payed = True

            if divida["valor"] < 0:
                payed = False
                next_pay = -divida["valor"]
                divida["valor"] = 0
